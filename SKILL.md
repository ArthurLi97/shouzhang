...
// ❌ 错误：导航栏和子页面同时显示标题
HdsNavigation()
  .titleBar({ content: { title: { mainTitle: 'SnapSticker' } } })
// IndexPage 内部又有一个 Row { Text('SnapSticker') }
...
- **技术栈**：ArkTS Strict Mode + ArkUI + HDS (`@kit.UIDesignKit`) + VisionKit + CameraKit + MediaLibraryKit

### A.2 颜色 Token

```typescript
// constants/ThemeConstants.ets
export const BRAND_PRIMARY      = '#FF8A80'   // 品牌珊瑚红
export const BRAND_PRIMARY_LIGHT = '#FFF0ED'  // 浅红背景
export const BRAND_SECONDARY    = '#FF6B6B'   // 辅色
export const BG_BASE            = '#FAF8F5'   // 页面底色
...
export const SEARCH_BG          = '#F2F2F2'   // 搜索框背景
export const CHIP_BG            = '#F2F2F2'   // 标签/Chip 背景
export const SHADOW_SOFT        = 'rgba(26, 26, 26, 0.06)' // 柔和阴影
```
...
| Symbol | 用途 |
|--------|------|
| `sys.symbol.house` | 贴纸库 Tab |
| `sys.symbol.paintbrush` | 创作板 Tab |
| `sys.symbol.camera` / `camera_fill` | 拍照、新建贴纸、滤镜占位 |
| `sys.symbol.plus` | 新建、添加贴纸、添加分类 |
| `sys.symbol.chevron_left` / `chevron_right` | 返回、展开 |
| `sys.symbol.xmark` | 关闭、删除图层 |
| `sys.symbol.arrow_counterclockwise` / `arrow_clockwise` | 撤销、重做 |
| `sys.symbol.arrow_up` | 导出、图层上移 |
| `sys.symbol.arrow_down` | 图层下移 |
| `sys.symbol.square_grid_2x2` | 贴纸库、网格、空状态 |
| `sys.symbol.textformat` | 文字工具 |
| `sys.symbol.rectangle` | 背景工具 |
| `sys.symbol.trash` | 删除贴纸、删除分类、清空画布 |
| `sys.symbol.heart` | 收藏、加入收藏 |
| `sys.symbol.gear` / `gearshape` | 设置、更多操作 |
| `sys.symbol.lock` | 隐私政策 |
| `sys.symbol.doc_text` | 用户协议 |
| `sys.symbol.questionmark_circle` | 帮助、未检测到主体 |
| `sys.symbol.info_circle` | 关于我们 |
| `sys.symbol.checkmark_circle_fill` | 选中态 |
| `sys.symbol.calendar` | 创建时间 |
| `sys.symbol.star` | 处理中动画（占位） |
| `sys.symbol.circle` | 滤镜选中态占位 |
| `sys.symbol.arrow_up_left_and_arrow_down_right` | 缩放操控柄 |

> ⚠️ **避坑**：未在本清单中列出的 Symbol 可能存在 tofu（方框）风险，使用前请先在真机或预览器上验证。

### A.4 HDS 封装组件

项目位于 `components/hds/` 目录，提供 HDS 风格的 ArkUI 封装，统一通过 `components/hds/Index.ets` 导出：

| 组件 | 路径 | 说明 |
|------|------|------|
| `HdsActionBar` | `components/hds/HdsActionBar.ets` | 顶部操作栏（startButtons / primaryText / endButtons） |
| `HdsListItem` | `components/hds/HdsListItem.ets` | 列表项（图标 + 主文本 + 后缀箭头） |
...
│       ├── BackgroundPage (编辑页背景选择)
│       ├── PrivacyPolicyPage (隐私政策)
│       ├── UserAgreementPage (用户协议)
│       └── SettingsPage (设置)
│
├── TabContent 1: 拍照（特殊 Tab，不切换页面）
│   └── 点击弹出 bindSheet（相机/相册选择）
│
└── TabContent 2: 创作板
...
SnapSticker 使用 **3 个 Tab**，中间拍照 Tab 为特殊样式（圆形凸起按钮）：
...
CameraTabBuilder(): void {
  Column({ space: 4 }) {
    ...
    Text('拍照').fontSize(10).fontColor($r('app.color.ink_mid'))
  }
  .width('100%')
  .height('100%')
  .justifyContent(FlexAlign.Center)
...
> 拍照 Tab **不切换页面**，点击时弹出新建贴纸选择器（bindSheet）。创作板（Tab 2）进入全屏编辑时，TabBar 自动隐藏（`barHeight(0)`）。

...
MEMO 是一款 HarmonyOS 原生应用，通过拍照将现实世界中的物体自动抠图生成贴纸，按 Collection 分类收藏，在纸张画布中自由组合创作，形成属于自己的视觉手账。
...
## 首页架构 · Collection First

MEMO 的首页以 **Collection（收藏）** 为主角，Canvas（画布）为配角：

```
┌─────────────────┐
│ June 3          │  ← 弱化顶部：日期 + "My Collection"
│ My Collection   │
├─────────────────┤
│                 │
│  Coffee    12   │  ← Collection 层叠纸张（主角，~55%）
│ ─────────────── │     每张纸有厚度、阴影、左侧标签脊
│  Food       8   │
│ ─────────────── │
│  Flower     5   │
...
│                 │
├─────────────────┤
│  [Mini Canvas]  │  ← 小画布预览（~25%），点击进入编辑
├─────────────────┤
│   [📷 Capture]  │  ← 底部大印章按钮（~15%）
└─────────────────┘
```

- **层叠 Collection 纸张**：每个 Collection 是一张厚纸板卡片，层叠排列，显示分类名称和贴纸计数（包括 0）
- **Mini Canvas 预览**：小尺寸纸张画布，显示当前 canvas 状态，点击进入完整编辑
- **Capture Button**：底部大印章按钮，像文具印章，拍照/选图制作新贴纸
- **Collection Detail**：点击 Collection 进入贴纸网格页，浏览该分类下的所有贴纸
...
│   ├── home/               # 首页 + 拍照/处理/预览流程
│   │   ├── HomePage.ets            # 🏠 首页：Collection层叠 + Mini Canvas + Capture
│   │   ├── CollectionDetailPage.ets# 📁 Collection 贴纸详情页（网格浏览）
...
│   │   └── ...                     # 其他旧页面（待清理）
│   ├── canvas/             # 画布创作模块（子页面）
│   │   ├── CanvasEditPage.ets      # ✏️ 画布编辑器（从首页进入）
│   │   ├── ExportPage.ets          # 导出分享
...
│   │   ├── CollectionFolderSheet.ets # 📂 Collection 层叠纸张卡片
│   │   ├── CollectionDrawer.ets    # Collection 抽屉（旧，保留兼容）
...
| **首页（Collection First）** | `pages/home/HomePage.ets` | ✅ 新架构 |
| Collection 贴纸详情 | `pages/home/CollectionDetailPage.ets` | ✅ 新页面 |
| 画布编辑器 | `pages/canvas/CanvasEditPage.ets` | ✅ 子页面 |
...
| 页面 | 路径 | 状态 |
|------|------|------|
| 首页（旧贴纸库） | `pages/home/HomePage.ets`（旧） | ❌ 旧 Tab 架构（已替换） |
